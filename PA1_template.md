...
On the complete dataset, create a factor column to report if the date of the measurement belongs to a weekday or a weekend. For that purpose use the function `wday()` from `lubridate` package, which returns the number 1 for sunday, 2 for monday, ... and 7 for saturday.
...
![plot of chunk plotDailyAverageWeekday](figure/plotDailyAverageWeekday-1.png) 
